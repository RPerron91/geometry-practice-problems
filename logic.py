...
def complete_squares():
    ...
    print("Your answer needs to be in the following format: a(x+b)^2 + c")
    print("If a, b, or c equal 1 or 0, please enter those values")
    print("If c is negative, please type your answer like this: a(x+b)^2 + (-c)")
    user_ans = input("Formula: ")
    a = min_num
    b = (mid_num/(2 * a))
    c = max_num - (mid_num**2 / (4 * min_num))
    if c < 0:
        answer = str(a) + f"(x+{b})^2" + f" + ({c})"
    else:
        answer = str(a) + f"(x+{int(b)})^2 + {int(c)}"
...
def center_circle():
    ...
    print("Your answer for the Center point needs to be in the format: (a, b)")
    print("All Numbers will be rounded to the tenth place")
    print("Radius will be the absolute value")
    center = input("Center: ")
    radius = input("Radius: ")
    user_answer = f"c = {center}, r = {radius}"
...
def bonus_tangent_secant_segment():
  ...
  if 'C' not in selected_segments:  # C is the missing segment
      segment_values['C'] = (segment_values['A']**2 - segment_values['B']**2) / segment_values['B']
  elif 'B' not in selected_segments:  # B is missing
      segment_values['B'] = max(solve_quadratic(1, segment_values['C'], (-1 * segment_values['A']**2)))
  else:  # A is missing
      segment_values['A'] = math.sqrt(segment_values['B']**2 + (segment_values['B'] * segment_values['C']))
  ...
  user_input = float(input(question + " "))  # Allow for decimal answers

  correct_answer = segment_values[segments[0] if 'A' not in selected_segments else segments[1] if 'B' not in selected_segments else segments[2]]
